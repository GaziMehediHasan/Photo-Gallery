<?php 
	...
	$sqlquery = 'SELECT * FROM image_info WHERE iowner="'.$_SESSION["session_eamil"].'"';
		...
		<div class="row">
			...
			<div class="col-md-10">
					...
					<div class="pagination">
						<?php
							...
							for ($i=1; $i <= $page_num; $i++) { 
								echo '<a class="'.activate($i).'" href="album.php?page='.$i.'">'.$i.'</a>';
							}
							echo '<a href="album.php?page='.$page_num.'">>></a>';
						?>
					</div>
				</div>
				...
				<div style="margin-left: 2%; margin-right: 2%;">
					<div class="row">
						<?php
							$sqlquery = 'SELECT * FROM image_info WHERE iowner="'.$_SESSION["session_eamil"].'"'.'LIMIT '.$first.','.$per_page;
							$result = mysqli_query($link,$sqlquery) or die(mysqli_error());
