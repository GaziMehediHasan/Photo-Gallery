<?php 
	...
	if (isset($_POST["delete"]) || isset($_POST["save"])) {
		if (empty($_POST["image_id"])) {
			$updateerror .= 'enter an image id';
		} else {
			$id = (int)clean_text($_POST["image_id"]);
		}
		
		if (empty($_POST["image_name"])) {
			$updateerror .= 'enter an image name';
		} else {
		...
		if ($updateerror == '') {
			...
			if (isset($_POST["delete"])) {
				...
				if (mysqli_query($link, $sql)) {
					...
					$updateerror .= 'unsuccessfully to delete image';
				}
			}
			...
			if (isset($_POST["save"])) {
				...
				if (mysqli_query($link, $sql)) {
					$updateerror .= 'Record update successfully';
					$updatestat = 1;
				} else {
					$updateerror .= 'update unsuccessfully';
				}
			}
		}
	}
